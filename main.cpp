...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (no == par.second) {
            if (!has_norte) {
                has_norte = true;
                norte = par;
            } else if (min_esq_dir_dist < min(abs(norte.first-es), abs(norte.first-di))) {
                norte = par;
            }
        }
        ...
        if (su == par.second) {
            if (!has_sul) {
                has_sul = true;
                sul = par;
            } else if (min_esq_dir_dist < min(abs(sul.first-es), abs(sul.first-di))) {
                sul = par;
            }
        }
        ...
        if (es == par.first) {
            if (!has_esq) {
                has_esq = true;
                esq = par;
            } else if (min_nor_sul_dist < min(abs(esq.second-no), abs(esq.second-su))) {
                esq = par;
            }
        }
        ...
        if (di == par.first) {
            if (!has_dir) {
                has_dir = true;
                dir = par;
            } else if (min_nor_sul_dist < min(abs(dir.second-no), abs(dir.second-su))) {
                dir = par;
            }
        }

        int curr_dist = max(min_nor_sul_dist, min_esq_dir_dist);
        ...
        if (!has_prox) {
            ...
            if (largura < altura) {
                if (min_esq_dir_dist < min(abs(prox.first - es), abs(prox.first - di))) {
                    prox = par;
                }
            } else {
                if (min_nor_sul_dist < min(abs(prox.second - no), abs(prox.second - su))) {
                    prox = par;
                }
            }
        }
    ...
    if (norte_sul < esq_dir) {
        altura_1 += min(k, norte_sul);
        altura_1 += k - min(k, norte_sul);
        largura_1 += k - min(k, norte_sul);
    } else if (norte_sul > esq_dir) {
        largura_1 += min(k, esq_dir);
        largura_1 += k - min(k, esq_dir);
        altura_1 += k - min(k, esq_dir);
    } else {
    ...
    if (prox != norte && prox != sul && prox != esq && prox != dir && prox_dist < k) {
        norte_dist = min(k, no - prox.second);
        esq_dist = min(k, prox.first - es);
        sul_dist = min(k, prox.second - su);
        dir_dist = min(k, di - prox.first);
